...
function toSql(data: SeedData): string {
  ...
  // Captions (text1 = language1, text2 = language2)
  const captionRows = captions.map(
    (c) =>
      `((SELECT id FROM videos WHERE youtube_id = '${esc(id)}'), ${c.idx}, ${c.begin}, ${c.end}, '${esc(c[video.language1 as "ko" | "en"])}', '${esc(c[video.language2 as "ko" | "en"])}')`,
  );
  ...
  for (const b of bookmarks) {
    ...
    lines.push(
      ...
      `  FROM videos v JOIN captions c ON c.video_id = v.id AND c.idx = ${cue.idx}`,
