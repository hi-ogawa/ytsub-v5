...
Preparing for first production deployment. The worker name needs to change from the project codename to the product name before going live, so the public URL is `zamak.hiro18181.workers.dev`.

## Code changes

1. **`wrangler.jsonc`** — `"name": "ytsub-v5"` → `"name": "zamak"`
2. **`vite.ext.config.ts:94`** — worker URL `ytsub-v5.hiroshi.workers.dev` → `zamak.hiro18181.workers.dev`
