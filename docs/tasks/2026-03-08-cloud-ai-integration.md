...
These are _total agent tokens_ (includes tool call overhead, reasoning, retries). A structured API pipeline would use fewer tokens since:
...
Note: the current agent runs on Claude Code (subscription), so these costs would be _new_ spend. But very manageable.
